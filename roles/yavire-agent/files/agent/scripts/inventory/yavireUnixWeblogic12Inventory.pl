...
#* NombreFichero: yavireUnixWeblogic12Inventory.pl
#*=========================================================
...
#*=============================================
#* Descripción:
#*    Genera el inventario diario de Weblogic 12 para Unix
#*
#*=============================================
#* Version: 2.2.0.2_1
#*
...
sub process_nodeGetServerPort {
  ...
  # <server>
    ...
    # </ssl>
    # <machine>yavire1</machine>
    # <listen-port>7001</listen-port>
    # <listen-port-enabled>true</listen-port-enabled>
    # <listen-address>dvyavtest1</listen-address>
           ...
           if ($type eq 'listen-port') {
              ...
              if ($bEstoyEnSSL == 0) {
                 ...
                 $bEncontradoPuerto = 1;
                  ...
                  print LOG "\nHE ENCONTRADO EL LISTEN-PORT DEL SSL $server : @$content[1] \n\n";
                  $bEncontradoPuerto = 0;
              }
	...
	local($xmlfile) = $_[0];
       ...
       print LOG "=====================================================\n";    
        ...
        if ($bEncontradoAdminInstance == 0) {
          print LOG "Admin Instance Name not found\n\n\n\n";
          $adminInstanceName="-";;
        }
        else {
            
             print LOG "Admin instance name found: $adminInstanceName \n\n";
          }
...
$ficheroInventarioDiario="yavireInv_weblogic12.data";
...
print "Ejecutando programa yavireUnixWeblogic12Inventory.pl\n";

# Si es una máquina windows.
# $^O : devuelve el sistema operativo
if ($^O =~ /Win/) {
    $szFichLog="$baseAgentDirWin\\log\\inventory\\yavireWinWeblogic12Inventory.log"; 
}
else {
   $szFichLog="$baseAgentDirUnix/log/inventory/yavireUnixWeblogic12Inventory.log"; 
...
print LOG "$fecha: Iniciando programa yavireUnixWeblogic12Inventory.pl $versionYAV\n";
...
if ($^O =~ /Win/) {
    ...
    $dirFichDataDiario="$baseAgentDirWin\\inventory\\data\\weblogic12\\$uuid\\"; 
  ...
  $rutaFicheroInventarioSemanal="$dirFichDataSemanal$ficheroInventarioSemanal"; 
    ...
    $dirFichDataDiario="$baseAgentDirUnix/inventory/data/weblogic12/$uuid/"; 
    $rutaFicheroInventarioDiario="$dirFichDataDiario$ficheroInventarioDiario"; 
    if (-d $dirFichDataDiario) {
       print LOG "El directorio $dirFichDataDiario existe\n";
    } 
...
while (<INVENTARIO_SEMANAL>) {	
	...
	next if ($esta_version !~ /12\./);
	
	print LOG "Es una instalacion de weblogic 12\n";
	...
	if ($^O =~ /Win/) {
        ...
        foreach (@servers) {
           ...
           #print INVENTARIO_DIARIO "versionYAV= ${versionYAV} \%\% instanceType= ${TipoInstancia} \%\% serverUUID= ${uuid} \%\%  serverVendor= ${system_manufacturer} \%\% serverName= ${system_name} \%\% serverIP= ${ip_server} \%\% productVendor= ${vendor} \%\% productName= ${producto} \%\% productVers= ${subversionWeblogic} \%\% instanceDomain= ${dominio} \%\% instanceName= ${instancia} \%\% instanceIP= ${ip_address} \%\% instancePort= ${instancePort} \%\% isAdminInstance= ${isAdmin} \%\% memoryMin= ${memoria_min} \%\% memoryMax= ${memoria_max} \%\% maxThreads= ${maxThreads} \%\% clusterInstance= ${cluster} \%\% propietary= ${propietario} \%\% LogAccess=${LogAccess} \%\% instanceDir= $directorio \%\%\n";
...
$fecha=yavireUnix::formatoFechaLog();
print LOG "$fecha: Finalizando programa yavireUnixWeblogic12Inventory.pl $versionYAV\n";
...
#*===========================================================
#* Fin script: yavireUnixWeblogic12Inventory.pl]
#*===========================================================
